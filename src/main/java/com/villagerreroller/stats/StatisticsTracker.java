...
public class StatisticsTracker {
    ...
    public StatisticsTracker() {
        Path configDir = FabricLoader.getInstance().getConfigDir().resolve(VillagerReroller.MOD_ID);
    ...
    private void loadStatistics() {
        ...
        if (file.exists()) {
            try (FileReader reader = new FileReader(file)) {
                ...
                VillagerReroller.LOGGER.info("Loaded statistics from {}", statsFile);
            } catch (IOException e) {
    ...
    public void recordSuccessfulReroll(int attempts) {
        ...
        VillagerReroller.LOGGER.info("Recorded successful reroll after {} attempts", attempts);
    }
    ...
    public void recordFailedReroll(int attempts) {
        ...
        VillagerReroller.LOGGER.info("Recorded failed reroll after {} attempts", attempts);
    }
    ...
    public double getSuccessRate() {
        ...
        return (double) allTimeStats.successfulRerolls / allTimeStats.totalRerolls * 100;
    }
    ...
    private static class Statistics {
        ...
        void recordBestTrade(int attempts) {
            TradeRecord record = new TradeRecord(System.currentTimeMillis(), attempts);
            bestTrades.add(record);

            // Keep only top 10 best trades (fewest attempts)
    ...
    public static class TradeRecord {
        ...
        public TradeRecord(long timestamp, int attempts, String itemName, int emeraldCost) {
            this.timestamp = timestamp;
            this.attempts = attempts;

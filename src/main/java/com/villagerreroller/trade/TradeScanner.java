...
public class TradeScanner {
    ...
    public List<ScannedTrade> scanCurrentTrades() {
        ...
        if (client.currentScreen instanceof MerchantScreen merchantScreen) {
            try {
                ...
                VillagerReroller.LOGGER.debug("Scanned {} trades from villager", trades.size());
            } catch (Exception e) {
    ...
    public static class ScannedTrade {
        ...
        public ScannedTrade(int slotIndex, TradeOffer offer) {
            ...
            this.buyItem = offer.getOriginalFirstBuyItem().copy();
        ...
        private static Map<RegistryEntry<Enchantment>, Integer> getEnchantmentsFromItem(ItemStack stack) {
        ...
        private int calculateEmeraldCost() {
            ...
            if (!secondBuyItem.isEmpty() && secondBuyItem.getItem() == Items.EMERALD) {
        ...
        public String toString() {
            return String.format("Trade[slot=%d, item=%s, emeralds=%d, enchants=%s]",
                    slotIndex, itemId, emeraldCost, getEnchantmentNames());
        }
    }
}

...
public class TradeFilter {
    ...
    public List<TradeScanner.ScannedTrade> filterTrades(List<TradeScanner.ScannedTrade> trades) {
        ...
        VillagerReroller.LOGGER.debug("Filtered {} trades from {} total", filtered.size(), trades.size());
        return filtered;
    }
    ...
    public boolean matchesCriteria(TradeScanner.ScannedTrade trade) {
        ...
        // Check item whitelist/blacklist
        if (!config.getItemWhitelist().isEmpty()) {
            conditions.add(config.getItemWhitelist().contains(trade.getItemId()));
        }

        if (!config.getItemBlacklist().isEmpty()) {
            conditions.add(!config.getItemBlacklist().contains(trade.getItemId()));
        }

        // Check price thresholds
        conditions.add(checkPriceThreshold(trade));

        // Check enchantment filters
        if (!config.getEnchantmentFilters().isEmpty()) {
            conditions.add(checkEnchantmentFilters(trade));
        }

        // Check combined enchantments
        if (!config.getCombinedEnchantments().isEmpty()) {
            conditions.add(checkCombinedEnchantments(trade));
        }

        // Check preferred first slot items
        if (!config.getPreferredFirstSlotItems().isEmpty() && trade.getSlotIndex() == 0) {
            conditions.add(config.getPreferredFirstSlotItems().contains(trade.getItemId()));
        }
        ...
        for (String filter : config.getEnchantmentFilters()) {
            for (String tradeEnchant : tradeEnchants) {
                if (matchesEnchantmentFilter(tradeEnchant, filter)) {
                    // Check if max level is required
    ...
    private boolean matchesEnchantmentFilter(String tradeEnchant, String filter) {
        ...
        return tradeEnchant.equals(filter) || tradeEnchant.startsWith(filter.split(":")[0]);
    }
    ...
    private boolean checkCombinedEnchantments(TradeScanner.ScannedTrade trade) {
        ...
        // Check if all required enchantments are present
